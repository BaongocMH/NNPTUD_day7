...
router.post("/import-excel", async function (req, res, next) {
  try {
    ...
    let filePath = req.body.filePath || process.env.IMPORT_USERS_FILE_PATH;
    if (!filePath) {
      return res.status(400).send({ message: "Missing filePath" });
    }
    let rows = readRowsFromExcel(filePath).filter(function (row) {
